...
import { 
  Mail, Lock, User, Gift, Eye, EyeOff, UserPlus, 
  Heart, Sparkles, ArrowRight, CheckCircle, Shield
} from 'lucide-react';
...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.success) {
      toast.success('Welcome to CloseConnect! 🎉');
      navigate('/dashboard');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-600 via-pink-500 to-orange-400 flex items-center justify-center p-3 sm:p-4 relative overflow-hidden">
      ...
      {/* Floating Hearts */}
      <Heart className="absolute text-white/10 w-12 h-12 sm:w-16 sm:h-16 top-10 left-10 animate-pulse" />
      <Heart className="absolute text-white/10 w-10 h-10 sm:w-12 sm:h-12 bottom-20 right-10 animate-pulse" style={{ animationDelay: '2s' }} />
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="bg-white/95 backdrop-blur-xl rounded-3xl shadow-2xl p-5 sm:p-6 md:p-8 border border-white/20">
          ...
          <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4">
            ...
            {/* Confirm Password */}
            <div className="relative">
              <Shield className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 sm:w-4 sm:h-4 text-gray-400" />
              <input
              ...
              {formData.confirmPassword && formData.password === formData.confirmPassword && (
                <CheckCircle className="absolute right-10 sm:right-12 top-1/2 -translate-y-1/2 w-3.5 h-3.5 sm:w-4 sm:h-4 text-green-500" />
              )}
            </div>

            {/* Invite Code */}
            <div className="relative">
              <Gift className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 sm:w-4 sm:h-4 text-gray-400" />
              <input
                type="text"
                name="inviteCode"
